    ...
    : AudealizeAudioProcessor (owner), mReverb ()
{
    ...
    mState->createAndAddParameter (paramD, prefix + "Delay of comb filters", TRANS ("Delay of comb filters"),
                                   mParamRange[kParamD], DEFAULT_D, nullptr, nullptr);
    mState->createAndAddParameter (paramG, prefix + "Gain of comb filters", TRANS ("Gain of comb filters"),
                                   mParamRange[kParamG], DEFAULT_G, nullptr, nullptr);
    mState->createAndAddParameter (paramM, prefix + "Delay between channels", TRANS ("Delay between channels"),
                                   mParamRange[kParamM], DEFAULT_M, nullptr, nullptr);
    mState->createAndAddParameter (paramF, prefix + "LP Cutoff", TRANS ("LP Cutoff"), mParamRange[kParamF], DEFAULT_F,
                                   nullptr, nullptr);
    mState->createAndAddParameter (paramE, prefix + "Effect Gain", TRANS ("Effect Gain"), mParamRange[kParamE],
                                   DEFAULT_E, nullptr, nullptr);

    mState->createAndAddParameter (paramAmountId, "Reverb: Amount", "Reverb: Amount",
...
void AudealizereverbAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    mReverb.init (mParamRange[kParamD].snapToLegalValue (DEFAULT_D), mParamRange[kParamG].snapToLegalValue (DEFAULT_G),
                  mParamRange[kParamM].snapToLegalValue (DEFAULT_M), mParamRange[kParamF].snapToLegalValue (DEFAULT_F),
                  mParamRange[kParamE].snapToLegalValue (DEFAULT_E),
                  mParamRange[kParamAmount].snapToLegalValue (DEFAULT_MIX), sampleRate);
    // debugParams();
...
bool AudealizereverbAudioProcessor::setPreferredBusArrangement (bool isInput, int bus,
...
{
    DBG ("\nREVERB: d: " << mReverb.get_d () << " g: " << mReverb.get_g () << " m: " << mReverb.get_m ()
                         << " f: " << mReverb.get_f () << " E: " << mReverb.get_E ());
    DBG ("PARAMS: d: " << mState->getParameter (paramD)->getValue ()
                       << " g: " << mState->getParameter (paramG)->getValue ()
                       << " m: " << mState->getParameter (paramM)->getValue ()
                       << " f: " << mState->getParameter (paramF)->getValue ()
                       << " E: " << mState->getParameter (paramE)->getValue ());
}
